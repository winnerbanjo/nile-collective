...
export const CartProvider = ({ children }) => {
  ...
  const loadUserCart = async (userId) => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL || 'https://nile-backend-9wdk.onrender.com'}/api/users/${userId}/cart`)
  ...
  const saveCart = async (items) => {
    ...
    if (userId) {
      try {
        await axios.put(`${import.meta.env.VITE_API_URL || 'https://nile-backend-9wdk.onrender.com'}/api/users/${userId}/cart`, {
          cart: items
        })
      } catch (error) {
