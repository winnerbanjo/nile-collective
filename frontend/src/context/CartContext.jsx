...
export const CartProvider = ({ children }) => {
  ...
  const loadUserCart = async (userId) => {
    try {
      const response = await axios.get(`https://nile-backend-9wdk.onrender.com/api/users/${userId}/cart`)
  ...
  const saveCart = async (items) => {
    ...
    if (userId) {
      try {
        await axios.put(`https://nile-backend-9wdk.onrender.com/api/users/${userId}/cart`, {
          cart: items
        })
      } catch (error) {
