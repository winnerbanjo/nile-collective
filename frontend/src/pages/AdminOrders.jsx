...
const AdminOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      console.log('Fetching orders from: https://nile-backend-9wdk.onrender.com/api/orders')
      setLoading(true)
      setError(null)
      const response = await axios.get('https://nile-backend-9wdk.onrender.com/api/orders')
  ...
  const handleStatusUpdate = async (orderId, newStatus) => {
    try {
      ...
      const response = await axios.put(`https://nile-backend-9wdk.onrender.com/api/orders/${orderId}/status`, {
        status: newStatus
      })
