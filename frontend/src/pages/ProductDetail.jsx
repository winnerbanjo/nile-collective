...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        console.log(`Fetching product from: http://localhost:5001/api/products/${id}`)
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:5001/api/reviews/product/${product._id}`)
